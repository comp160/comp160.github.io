...
based on the steps outlined in the previous section. We will call our recurrence
$$M(n)$$:

1. Dividing $$A$$ into subarrays would take $$\Theta(n)$$ since we need to iterate through
all elements in $$A$$ &#8594; this step takes $$\Theta(n)$$
2. Sorting each subarray of size 5 would take $$\Theta(5 log 5) = \Theta(1)$$, and since
we have $$\frac{n}{5}$$ groups, our total time is $$\frac{n}{5}\cdot\Theta(1) = \Theta(n)$$
&#8594; this step takes $$\Theta(n)$$
3. Recursively compute the median-of-medians requires calling the median-of-medians
algorithm again on the list of medians &#8594; this step would be $$M(\frac{n}{5})$$
4. After getting the median-of-medians, we use it to partition the entire array, which
requires iterating through all elements &#8594; this takes $$\Theta(n)$$
5. This step is just checking a simple inequality &#8594; this takes $$\Theta(1)$$
6. The final recursive step calls median-of-medians on the portion of the array that
we need to recurse on. In the worst case, this would be roughly $$\frac{3n}{4}$$ &#8594;
this step would be $$M(\frac{3n}{4})$$

So at each call, the work we're doing comes from steps 1, 2, 4, and 5. Since steps 1, 2, and
4 takes $$\Theta(n)$$ and step 5 takes $$\Theta(1)$$, adding these up would result in $$\Theta(n)$$
work at each call.

We also make 2 recursive calls, $$M(\frac{n}{5})$$ to find the median-of-medians, and 
$$M(\frac{3n}{4})$$ to recursively find the element we're looking for. This gives us the following
recurrence: $$M(n) = M(\frac{n}{5}) + M(\frac{3n}{4}) + \Theta(n)$$.

### The recursion tree explanation

### The formal substitution proof